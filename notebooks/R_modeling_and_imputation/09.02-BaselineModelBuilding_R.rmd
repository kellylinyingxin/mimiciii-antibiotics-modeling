...
###note: a big change was made after realizing i had used the PA projec tpatient list, rather than the prolonged ab patient list. model building results will vary now.
...
```{r}
data<- read.csv("/Users/geickelb1/Documents/GitHub/mimiciii-antibiotics-modeling/models/imputation/01042019_median_imputed_train.csv", stringsAsFactors=TRUE,check.names=TRUE)
data_test<- read.csv("/Users/geickelb1/Documents/GitHub/mimiciii-antibiotics-modeling/models/imputation/01042019_median_imputed_test.csv", stringsAsFactors=TRUE,check.names=TRUE)
...
full_imputed<-factor_convert(full_imputed, level_number=4)
...
### cart tree
```{r}
#CT-First fit tree with all data
library(rpart)
set.seed(12345)
control <- rpart.control(minbucket = 1, cp = 0.0001, maxsurrogate = 0, usesurrogate = 0, xval = 10)
fit.tr <- rpart(final_bin~., preprocessed_imputd, method = "class", control = control) #method=anova for regression tree. 
plotcp(fit.tr)
printcp(fit.tr)  #same info in FGL.tr$cptable

#prune to optimal size
bestcp <- fit.tr$cptable[which.min(fit.tr$cptable[,"xerror"]),"CP"]
fit.tr1 <- prune(fit.tr, cp=bestcp)  #approximately the cp corresponding to the optimal size after reapeating above code a few times
par(cex=0.5); plot(fit.tr1, uniform=F); text(fit.tr1, use.n = F); par(cex=0.5)
printcp(fit.tr1) #pruned tree

#calculate training and CV misclass rates
print("training and cv misclass rates")
fit.tr1$cptable[nrow(fit.tr1$cptable),c(3,4)]*min(table(preprocessed_imputd$final_bin)/nrow(preprocessed_imputd))  #training and cv misclass rates


##this is the training misclass rate across all classes
yhat.tr2<-predict(fit.tr1, type="class")
print("tree misclassification rate: ")
sum(yhat.tr2 != preprocessed_imputd$final_bin)/nrow(preprocessed_imputd) #check the training misclass rate

```

cv misclass error: 12.1%, training misclass: 10.8%. 

variable importance:
```{r}
data.frame(fit.tr1$variable.importance)
```
...
table(preprocessed_imputd$final_bin)/nrow(preprocessed_imputd)
...
# w<- 1/table(preprocessed_imputd$final_bin)
# 
# preprocessed_imputd[,names(preprocessed_imputd) !="final_bin"]
# rFtune<- tuneRF(preprocessed_imputd[,names(preprocessed_imputd) !="final_bin"],  y=preprocessed_imputd$final_bin,
#                 #mtryStart=100,
...
rForest <- randomForest(x=preprocessed_imputd[,-c(46)],
                        y=preprocessed_imputd$final_bin,
...
confusionMatrix(rForest$predicted,reference=preprocessed_imputd$final_bin, positive="pos")
...
ranger1<- ranger(final_bin~.,preprocessed_imputd, case.weights=weights, replace=FALSE)
ranger1
ranger2<- ranger(final_bin~.,preprocessed_imputd, case.weights=weights, replace=TRUE)
ranger2

ranger3<- ranger(final_bin~.,preprocessed_imputd,  replace=TRUE)
ranger3
...
ranger2_test<-confusionMatrix(predict(ranger2,preprocessed_imputd_test)$predictions, reference= preprocessed_imputd_test$final_bin, positive="pos" );ranger2_test
ranger3_test<-confusionMatrix(predict(ranger3,preprocessed_imputd_test)$predictions, reference= preprocessed_imputd_test$final_bin, positive="pos" );ranger3_test
```
