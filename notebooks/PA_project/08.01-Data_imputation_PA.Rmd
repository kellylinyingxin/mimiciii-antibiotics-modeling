...
final_pt[is.na(final_pt['Gram_pos']),'Gram_pos']='cultureNeg'
```
...
worst2_sid= merge(worst2,final_pt[c('icustay_id','subject_id', 'Gram_pos','iCulture')], by='icustay_id', all.x=TRUE) #left join
#worst2_sid$Var.2 <- NULL
worst2_sid<- worst2_sid[,!(names(worst2_sid) %in% c('Var.2'))] #dropping a weird var.2. 

#worst2_sid[is.na(worst2_sid['iGram_pos']),'iGram_pos']='cultureNeg'
```
...
```{r}
summary(final_pt$iCulture)
```

```{r}
summary(final_pt$iGram_pos)
```



```{r}
summary(subset(trainSet, iCulture==T)$pco2)
summary(subset(trainSet, iCulture==F)$pco2)
```
...
imp2 <- mice(testSet, m=20, pred=quickpred(testSet, minpuc=0.25,
                                        exclude=c('ethnicity','final_bin','subject_id',
                                                  'pco2', "minCalcium",  "maxCalcium", 
                                                  'bilirubin', 'lactate','icustay_id',
                                                  'iCulture','Gram_pos')),
...
--------------
## tree model on data with missing values

```{r}


#first trying to classify culture +/-
trainSet_tree<- trainSet[, !names(trainSet)%in% c('icustay_id','subject_id','Gram_pos')]
glimpse(trainSet_tree)
...
trainSet_tree<- trainSet[, !names(trainSet)%in% c('icustay_id','subject_id','Gram_pos')]
up_train <- upSample(x = trainSet_tree[, !names(trainSet_tree)%in% c('iCulture')],y = trainSet_tree$iCulture)         
...
FGL.tr <- rpart(Class~., up_train, method = "class", control = control) #method=anova for regression tree. 
plotcp(FGL.tr)
#printcp(FGL.tr)  #same info in FGL.tr$cptable

#prune to optimal size
FGL.tr1 <- prune(FGL.tr, cp=0.0013)  #approximately the cp corresponding to the optimal size
FGL.tr1
par(cex=0.5); plot(FGL.tr1, uniform=F); text(FGL.tr1, use.n = F); par(cex=0.5)
FGL.tr1$variable.importance

#errors line
printcp(FGL.tr1) #pruned tree
FGL.tr1$cptable[nrow(FGL.tr1$cptable),] 

#calculate training and CV misclass rates
print("training and cv misclass rates")
FGL.tr1$cptable[nrow(FGL.tr1$cptable),c(3,4)]*min(table(up_train$Class)/nrow(up_train))  #training and cv misclass rates

yhat<-predict(FGL.tr1, type="class")
sum(yhat != up_train$Class)/nrow(up_train) #check the training misclass rate

```

```{r}
pred = predict(FGL.tr1, type="class")
table(pred, up_train$Class)
```
