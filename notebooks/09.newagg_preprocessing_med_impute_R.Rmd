...
note: this is a slightly modified median imputation preproessing document where I'm working with the new data aggregations (std, mean, and range) for each continuous variable.
```{r}
...
colnames(newagg)[colnames(newagg)=="bands"] <- "bandsNum"
colnames(newagg)[colnames(newagg)=="pao2/fio2"] <- "pao2fio2Ratio"


##changing 90 to be max age instead of 300
newagg$first_admit_age[newagg$first_admit_age>90]<- 90

newagg$dopamine<- as.factor(newagg$dopamine == 1)
newagg$dobutamine<- as.factor(newagg$dobutamine == 1)
newagg$vasopressin<- as.factor(newagg$vasopressin == 1)
newagg$epinephrine<- as.factor(newagg$epinephrine == 1)
newagg$norepinephrine<- as.factor(newagg$norepinephrine == 1)
newagg$phenylephrine<- as.factor(newagg$phenylephrine == 1)
newagg$rrt<- as.factor(newagg$rrt == 1)
...
newagg$ibands<- 'absent'
newagg[which( newagg$bandsNum<=10 & is.na(newagg$bandsNum)==FALSE) , 'ibands'] <- '<10'
newagg[which( newagg$bandsNum >10 & is.na(newagg$bandsNum)==FALSE) , 'ibands'] <- '>10'
summary(as.factor(newagg$ibands))


newagg$ipco2<- 'absent'
newagg[which( newagg$pco2<=50 & is.na(newagg$pco2)==FALSE) , 'ipco2'] <- '<50'
newagg[which( newagg$pco2 >50 & is.na(newagg$pco2)==FALSE) , 'ipco2'] <- '>50'
summary(as.factor(newagg$ipco2))

newagg<- newagg[ , !names(newagg) %in% c('pco2','bandsNum')]
...
```{r}
write.csv(median_imputed, file ="/Users/geickelb1/Documents/GitHub/mimiciii-antibiotics-modeling/models/imputation/04042019_newagg_median_imputed_train.csv", row.names=FALSE) #"/Users/geickelb1/Documents/GitHub/PAII_mimic_project/processed_data/20190221_median_imputed.csv")
...
```{r}
write.csv(median_imputed_test, file ="/Users/geickelb1/Documents/GitHub/mimiciii-antibiotics-modeling/models/imputation/04042019_newagg_median_imputed_test.csv", row.names=FALSE)
```



###STOPPED HERE 4-12-19. need to figure out how I want to deal with normalization from here on out.
