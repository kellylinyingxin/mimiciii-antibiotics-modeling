...
--8/28/18, trying to add charttime to all select statements

DROP MATERIALIZED VIEW IF EXISTS rrt_all_time CASCADE;
CREATE MATERIALIZED VIEW rrt_all_time as
with cv as
(
...
(
  ...
  group by ie.icustay_id, tt.charttime
)
, mv_pe as
(
    select ie.icustay_id, tt.starttime
...
select ie.subject_id, ie.hadm_id, ie.icustay_id, cv.charttime cv, mv_ce.charttime mv_ce, mv_ie.starttime mv_ie, mv_de.charttime mv_de, mv_pe.starttime mv_pe
  , case
...
--order by ie.icustay_id, cv, mv_ce.charttime, mv_ie.starttime, mv_de.charttime, mv_pe.starttime;